...
| [UNGRD RUD](https://rud.gestiondelriesgo.gov.co/) (`/home/json.php?temp=2026T`) | **La primera fuente oficial que cubre el evento**: damnificados por municipio (familias, personas, viviendas), cargado por autoridades locales | Público de lectura, NO documentado (descubierto 16-ago); **captura cerrada el 18-sep-2026** (`ungrd_rud.CAPTURA_CERRADA`, ver `docs/DECISIONES.md`): última captura, la del 17-sep |
| [Sedes educativas MEN (SISE)](https://mineducacion.maps.arcgis.com/apps/dashboards/5e47f09f3b374396a5b3be15e8e96192) (capa ArcGIS `SISE202608_Priorizadas_Final`) | **La primera fuente oficial sede a sede**: 9.273 sedes educativas con matrícula y estado físico declarado tras el sismo (28-ago-2026: 17 en colapso total, 113 en riesgo inminente de colapso, 104 en colapso parcial, 288 con afectación parcial, 306 con afectación menor, 159 que reportan afectación sin definir el impacto, 197 sin afectación). Donde el EDAN dice «12 centros educativos», esto dice cuáles y dónde — cuando trae coordenada: 2.190 llegan con el (0,0) de geocodificación de baja confianza y se tratan como sin coordenada. **8.089 de 9.273 están en «No aporta información» — sin verificar, que no es «sin daño»** | Público, sin clave; licencia no declarada (el item ArcGIS publica `licenseInfo` vacío, verificado 28-ago). **La capa se republica y muta en horas**: el 28-ago pasó de ~50.000 filas y 7 categorías a 9.273 y 8 entre dos sondas — comprobación diaria obligatoria, archivando solo lo que cambia; vigilada por test de supuesto |
...
| [Microsoft AI for Good Lab](https://data.humdata.org/organization/microsoft-ai4g-lab) (HDX, API CKAN `package_show`) | **La cuarta mirada satelital, y la única sobre el parque de edificios COMPLETO**: 882.805 edificios evaluados por IA en Cali y Pereira (dos fuentes de huellas —Google, Overture— más una reedición «Extended» de Pereira con revisión humana), no un recorte fotointerpretado a mano. El daño real es 0,3-0,5 % del total; `msft_danos` publica solo lo informativo (dañado, revisado por un humano o con media nube o más encima) — el censo completo cuenta agregado en `msft_recursos` ([por qué](docs/DECISIONES.md)) | Público, sin clave; licencia CC-BY. Los 7 GeoPackage y 2 GeoTIFF (268 MB) se archivan en R2, no en git; las máscaras de área válida sí van a git. La lista de recursos se lee de `package_show` en cada corrida, nunca de una URL fija (la firmada caduca) |
...
Tres canales, mismo contenido (balances en medios y cambios del propio
monitor — nivel alta y pulso diario), con un solo punto de envío y dedupe.
Los cambios del RUD salían por aquí hasta que su captura se cerró
(18-sep-2026, `ungrd_rud.CAPTURA_CERRADA`):
